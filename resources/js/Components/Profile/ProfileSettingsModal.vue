  ...
  >
    <div class="flex w-full max-w-xl flex-col rounded-[28px] border border-emerald-100 bg-white shadow-[0_45px_120px_-80px_rgba(16,185,129,0.6)]">
      <div class="relative border-b border-emerald-50 px-6 pt-6 pb-4 text-center text-emerald-800">
        ...
        <h2 class="text-xl font-semibold md:text-2xl">Kelola Profil</h2>
        <p class="text-xs text-emerald-600/80 md:text-sm">Ringkasan akun dan tindakan cepat.</p>
      </div>

      <div class="max-h-[65vh] overflow-y-auto px-6 pb-6 pt-6">
        <div class="space-y-6">
          <div class="text-center space-y-2">
            <div class="text-xs uppercase tracking-[0.35em] text-emerald-500">Pengguna aktif</div>
            <div class="text-lg font-semibold text-emerald-700">{{ name }}</div>
          </div>
          ...
          <dl class="space-y-3 text-sm text-slate-700">
            ...
            <div class="flex flex-col gap-1 border-b border-emerald-100 pb-3">
              <dt class="text-xs uppercase tracking-wide text-slate-400">NIP</dt>
              <dd class="font-medium">{{ nip }}</dd>
            </div>
            <div class="flex flex-col gap-1">
              <dt class="text-xs uppercase tracking-wide text-slate-400">Telepon</dt>
      ...
      <div class="flex items-center justify-between gap-3 border-t border-emerald-50 px-6 py-5">
        ...
        <button
          ...
          :disabled="loggingOut"
        >
          <span v-if="loggingOut" class="inline-block h-4 w-4 animate-spin rounded-full border-2 border-white border-r-transparent"></span>
